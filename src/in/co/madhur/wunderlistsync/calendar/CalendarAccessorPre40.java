 ...
 */
package in.co.madhur.wunderlistsync.calendar;
...
public class CalendarAccessorPre40 implements CalendarAccessor
{
	...
	public boolean addEntry(long calendarId, Date when, int duration, String title, String description)
	{
		if (App.LOCAL_LOGV)
		{
			Log.v(App.TAG, String.format("addEntry(%d, %s, %d, %s, %s)", calendarId, when.toString(), duration, title, description));
		}
	...
	public Map<String, String> getCalendars()
	{
		...
		try
		{
			cursor = resolver.query(Uri.withAppendedPath(CALENDAR, "calendars"), new String[] {
					"_id", "displayname" }, null, null, "displayname ASC");
