...
public class ListPreferenceHelper
{
	...
	public static boolean initListPreference(final ListPreference pref, final Map<?, ?> fields, boolean keepExisting)
	{
		if (fields.size() > 0)
		{
			...
			if (keepExisting)
			{
				if (pref.getEntries() != null)
					e.addAll(Arrays.asList(pref.getEntries()));
				if (pref.getEntryValues() != null)
					ev.addAll(Arrays.asList(pref.getEntryValues()));
			}
		...
		pref.setOnPreferenceChangeListener(new OnPreferenceChangeListener()
		{
			...
			{
				pref.setTitle(pref.getEntries()[pref.findIndexOfValue(newValue.toString())]);
				return true;
			}
		});
